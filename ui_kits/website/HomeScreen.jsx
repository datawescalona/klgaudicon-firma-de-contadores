...
function Hero({ go }) {
  ...
  return (
    <section className="klg-section klg-section--flush" style={{ position: 'relative', overflow: 'hidden', background: 'var(--navy-950)', marginTop: -88, justifyContent: 'flex-start' }}>
      ...
      {/* Hero body — centered vertically between nav and franja */}
      <div style={{ position: 'relative', zIndex: 2, flexGrow: 1, display: 'flex', alignItems: 'center', paddingTop: 104 }}>
        <div className="klg-pad" style={{ maxWidth: 'var(--container)', margin: '0 auto', padding: '0 var(--gutter)', width: '100%' }}>
          <div style={{ maxWidth: 820 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '7px 14px', borderRadius: 'var(--radius-pill)', border: '1px solid rgba(194,162,99,0.45)', background: 'rgba(168,132,74,0.12)', marginBottom: 26 }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--gold-500)' }}></span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--gold-300)' }}>Firma de Auditores Financieros · Caracas, Venezuela</span>
            </div>
            <h1 style={{ fontSize: 'clamp(2.4rem, 5vw, 4.4rem)', lineHeight: 1.06, color: '#fff', margin: 0, letterSpacing: '-0.025em' }}>
              Tu empresa no necesita<br />más contadores. Necesita<br /><RotatingWord />
            </h1>
            <p style={{ fontSize: 'var(--fs-lead)', lineHeight: 1.6, color: 'rgba(255,255,255,0.78)', maxWidth: 560, marginTop: 22 }}>
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 30, flexWrap: 'wrap' }}>
              ...
              <Button variant="ghostLight" size="lg" onClick={() => go('servicios')}>Ver servicios</Button>
            </div>
          </div>
        </div>
      </div>

      {/* Trust franja — pinned at the bottom of the hero viewport */}
      <div style={{ position: 'relative', zIndex: 2, background: 'rgba(7,10,24,0.55)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', borderTop: '1px solid rgba(194,162,99,0.28)' }}>
        <div className="klg-pad klg-franja" style={{ maxWidth: 'var(--container)', margin: '0 auto', padding: '20px var(--gutter)', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
          {trust.map(([ic, t], i) => (
            <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '4px 22px', borderLeft: i === 0 ? 'none' : '1px solid rgba(255,255,255,0.12)' }}>
              <span style={{ width: 38, height: 38, flexShrink: 0, borderRadius: 'var(--radius-sm)', background: 'rgba(168,132,74,0.16)', color: 'var(--gold-500)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}><Icon n={ic} s={19} /></span>
              <span style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.9)', fontWeight: 600, lineHeight: 1.3 }}>{t}</span>
            </div>
          ))}
        </div>
      </div>
